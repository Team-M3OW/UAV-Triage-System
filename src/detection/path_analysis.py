...
def compute_normalized_ratio(trajs_np, visibs_np, max_cap=5.0):
    ...
    for n in range(N):
        ...
        ratio = min(ratio, max_cap)
        norm_ratio = (ratio - 1) / (max_cap - 1)
        repetition_ratios.append(norm_ratio)

    return np.array(repetition_ratios)


def plot_normalized_ratios(ratios, save_path="normalized_ratio.png"):
