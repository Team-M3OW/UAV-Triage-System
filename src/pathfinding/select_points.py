    ...
    with rasterio.open(dem_path) as src:
        try:
            if src.crs.is_geographic:
                print("\nWARNING: DEM is in Geographic CRS. Reproject to Projected CRS recommended.")
                return False
            elif src.crs.is_projected:
                print("\nCRS Check: OK. DEM is in a Projected Coordinate System.")
                return True
            else:
                print("\nWarning: Could not determine CRS type.")
                return True
        except CRSError:
            print("\nERROR: DEM has no recognizable CRS. Cannot proceed.")
            sys.exit(1)
...
class PointSelector:
    ...
    def on_click(self, event):
        ...
        if self.mode == 'base':
            ...
            print(f"Casualty #{len(self.casualties)} selected at: {self.casualties[-1][0]:.2f}, {self.casualties[-1][1]:.2f}")
    ...
    def update_title(self):
        if self.mode == 'base':
            self.ax.set_title("Click to select NDRF Base. Close window when done.", fontsize=14)
        else:
            self.ax.set_title(
                f"Click to select Casualty locations ({len(self.casualties)} selected). Close window when done.",
                fontsize=14,
            )
